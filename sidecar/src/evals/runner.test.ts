...
import { type ApprovalRequest, executeTool, resolveApproval, ToolPolicyDenied } from "../tools.js";
...
describe("evals determinísticos (#211) — camada 1 de CI", () => {
  it(`executa as ${TOOL_TASKS.length} tarefas de ferramentas com denominadores explícitos`, async () => {
    expect(TOOL_TASKS.length).toBe(34);
    const results: TaskResult[] = [];
    let improperCardsInAutomatic = 0;
    const readOnlyMutations = 0;
    const escapes = 0;
